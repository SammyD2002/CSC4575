...
# An EVIL Server that flips one (1) bit of the payload.
from random import sample
# For each byte, flip a coin. If heads, flip the byte.
def nefarious(ciphertext:bytes)->bytes:
    tampered = bytearray(ciphertext)
    toflip = sample(range(len(ciphertext)), (int.from_bytes(os.urandom(1)) % len(ciphertext)) + 1)
    print(f'[+] Nefariously flipping {len(toflip)} bytes of the ciphertext...')
    for i in toflip:
        tampered[i] = ciphertext[i] ^ 0xFF
...
def encrypt_payload(plaintext: bytes, key: bytes) -> tuple[bytes, bytes, bytes]:
    ...
    return nonce, nefarious(ciphertext), incripter.tag
